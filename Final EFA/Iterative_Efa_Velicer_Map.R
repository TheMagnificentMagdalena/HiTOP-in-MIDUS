...
# This function uses Velicer MAP (psych::VSS) to determine number of factors. 
...
# loading_threshold = |0.30|, communal_threshold = 0.25. Logs each iteration.
run_iterative_efa <- function(data_subset, id) {
  
  items <- colnames(data_subset)
  loading_threshold  <- 0.30  # checked against absolute loadings
  ...
  R_full <- pc$rho
  if (is.null(colnames(R_full))) {
    colnames(R_full) <- rownames(R_full) <- colnames(data_subset)
  }
  n_obs <- nrow(data_subset)
  
  repeat {
    # Subset the precomputed correlation matrix for current items
    Rsub <- R_full[items, items, drop = FALSE]
    
    # Velicer MAP on Rsub
    vss_out <- VSS(
      Rsub,
      n.obs  = n_obs,
      n      = min(30, length(items)),
      rotate = "none"
    )
    nf <- which.min(vss_out$map)
...
# Save 
write.csv(combined_log, "EFA_removal_log_all_imputations_Velicer_MAP_Final.csv", row.names = FALSE)
