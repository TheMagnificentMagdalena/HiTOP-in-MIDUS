...
# CFA model -----------------------------------------------
#Updated to remove items loadings below .40
cfa_model <- '
...
fit_mi <- lavaan.mi::cfa.mi(
  ...
  data             = imp_list,
  estimator        = "WLSMV",
  parameterization = "delta",   #can also use theta but delta is more stable
  std.lv           = TRUE,
...
# Outputs and summary ----------------------------------------------------
{
  ...
  writeLines(sum_txt, "Pooled_Empirical_Model_CFA_summary.txt")
}
...
write.csv(pe, "Pooled_Empirical_Model_CFA_parameters_standardized.csv", row.names = FALSE)
...
write.csv(rel_df, "Pooled_Empirical_Model_CFA_reliability.csv", row.names = FALSE)

# Modification Indices (pooled) ------------------------------------------
mi <- lavaan.mi::modindices.mi(fit_mi, sort. = TRUE)
write.csv(mi, "Pooled_Empirical_Model_CFA_modindices.csv", row.names = FALSE)
...
# Write to CSV
write.csv(Phi_cov, "Pooled_Empirical_Model_CFA_phi_covariances.csv", row.names = TRUE)
write.csv(Phi_cor, "Pooled_Empirical_Model_CFA_phi_correlations.csv", row.names = TRUE)
...
cat("FACTOR SCORE INDETERMINACY ANALYSIS\n")
...
create_decision_matrix <- function(determinacy, reliability, uncertainty) {
  ...
  decision_matrix <- determinacy %>%
    ...
    mutate(Determinacy_r2 = round(Determinacy^2, 3)) %>%
...
# Save results
write.csv(decision_matrix, "Factor_score_empirical_decision_matrix.csv", row.names = FALSE)
